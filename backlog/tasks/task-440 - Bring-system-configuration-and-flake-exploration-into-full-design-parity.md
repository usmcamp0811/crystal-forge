...
2026-09-17 (session 2) automatic Config value preview + notification dismiss-all pass authorized at exact HEAD `bf27c03d03f15355a967960c5cc666b880b01064` (origin matches, worktree clean apart from the preserved generated `packages/web-ui/assets/tailwind.css`; the CVE-lease stash remains untouched): (1) add additive migration 0268 giving `config_observation_requests` an `is_automatic` column (kind='option' only, via CHECK), promote an existing queued/waiting automatic row to explicit on reuse, and order reservation by (priority, is_automatic, ...) so explicit work is never queued behind pending automatic previews while reusing the existing single interactive semaphore/advisory lock for capacity; (2) give only automatic-intent executions a new short ~10s `AUTOMATIC_OBSERVATION_DEADLINE`, leaving explicit inspection on the existing 5-minute deadline, and thread `automatic` through `CreateConfigObservationRequest` on both server and frontend with a kind=Option-only guard in the handler; (3) refactor `select_option` to share a new `ensure_option_detail(path, intent)` path-scoped loader; add a frontend-only `ObservationState::Deferred` outcome so an automatic attempt that fails or times out shows a quiet 'Not loaded automatically — select to inspect.' state (never styled as an error and never nonexistent) while a fresh explicit click always retries with the full deadline; automatic attempts never overwrite a Loaded/Lifecycle/Error/Deferred entry (no auto-retry, no duplicate of a shared in-flight fetch); (4) add a bounded, debounced, single-flight automatic scheduler driven by one shared IntersectionObserver scoped to `.cfgx-scroll` (matching the existing `hooks::infinite_scroll` sentinel-attribute convention) that tracks genuinely intersecting Option rows across Browse and Configured, recomputed fresh each debounce settle from the current visible-and-uncached set (bounded window `MAX_AUTOMATIC_PENDING_CANDIDATES = 16`), explicitly pruned on branch collapse and on revision/system change, and never expanding a prefix or requesting provenance/inventory/Configured-index to discover candidates; (5) add real dismiss-all notification support: `dismiss_all_notifications` query reusing the exact existing visibility predicate and a single set-based UPDATE with a NOW()-at-execution cutoff so later-arriving notifications survive, a matching authenticated+CSRF handler and route, a frontend client call, a `NotificationMutation::DismissAll` reducer path in the existing mutation-epoch feed exactly mirroring `mark_all_succeeded` (clear vs mark-read are kept semantically distinct), and topbar footer/list parity with the design reference (space-between Dismiss all/Notification settings footer, Load more relocated into the scrollable list, help text clarifying dismissal is personal and does not resolve underlying issues); (6) add focused pure-Rust tests for the new reducer/candidate-selection/priority logic, an isolated PostgreSQL test for `is_automatic` promotion/ordering and for `dismiss_all_notifications` scope and survival of later notifications, scoped rustfmt/cargo check, WASM/web-ui package build, and live-preview browser verification with delayed/failed fixture responses at matching viewports before/after screenshots; (7) update `docs/config-explorer-architecture.md` and the alerts/notifications doc to distinguish shallow tree identity loading, bounded automatic value preview, explicit expensive inspection/provenance, and personal notification dismissal versus reading/resolution; (8) deliver as two independently reviewable commits ('TASK-440: Load visible Config option values progressively' and 'TASK-440: Complete notification dismissal and design parity'), then commit and push normally to the existing branch/MR without merging, deploying, or running/monitoring the authoritative Web UI VM or CI per explicit maintainer instruction.

2026-09-17 notification dropdown correction: preserve the existing task worktree at actual HEAD `5c7537af` and untracked generated Tailwind asset; move the existing server-backed Dismiss all action to the reference-aligned footer while retaining the existing Mark all read handler in the header; place Load more as a continuation row inside the scrollable list; anchor the panel below the actual bell container with scoped CSS/DOM positioning that survives scroll, resize, and narrow viewports; align notification-only geometry and type styles with `Shell.jsx`/`styles.css`; update focused frontend/browser contracts; regenerate served CSS, inspect desktop/light/dark/narrow fixture screenshots and computed assets, run scoped formatting/tests/WASM checks/diff checks, then review, commit, and push without deployment, database mutation, or authoritative Web UI VM/CI monitoring.
...
2026-09-18 P1 evaluation-failure root cause PROVEN and fixed locally as commit `7c7d7e5d` (not pushed). The fatal error was not a Campground defect and not an evaluator/discovery parity bug. Crystal Forge commit `a6be7fa0` (this MR branch, absent from origin/dev) added `checks/verified-source-evaluator-parity/fixture/.gitattributes` containing `revision.txt export-subst`. That file is tracked inside the Crystal Forge repository, so the attribute also applied to Crystal Forge's own `git archive` output. `export-subst` expands `$Format:%H$` only under archive semantics, so a single Crystal Forge commit has two valid trees: archive semantics (used by `gitlab:`/`github:` flake fetchers and the GitLab archive API) yields `revision.txt = <sha>`, and clone/checkout semantics (used by the `git+https://` fetcher) yields the literal `$Format:%H$`. Measured for rev 465d6913: archive tree = sha256-n6J/F/7DsbvGSp05v1KDKvZA7L8oYUXZl3PNtFmURTo=, checkout tree = sha256-89vHDsyBx7PqBbUO/fwpRTekMwSuHmLtiT6Mo6bhyLA=. Campground's flake.lock node `crystal-forge_2` (original ref `TASK-440-system-config-flake-parity`) recorded the checkout hash while declaring `"type": "gitlab"`, so any host that must actually fetch the input failed with `NAR hash mismatch in input`. Reproduced verbatim in an isolated chroot store with `builtins.fetchTree`. Exactly one file differed between the two trees (`checks/verified-source-evaluator-parity/fixture/revision.txt`); file lists were identical. The last good lock (rev 55cc0dc, hash sha256-sfej8R9/4JaeUsubwr4b0h6nS4Q5gaycAkf3T2+vDQI=) matched archive semantics, so the divergence only bites when a consumer locks with clone semantics. Fix creates the `export-subst` attribute inside the throwaway fixture repository at check time and removes the tracked file, so the check's archive assertions are preserved and Crystal Forge's archive is byte-identical to its checkout (verified: both semantics now hash to sha256-IDJ+UZvzL0tcGcWaeD0O4D4/6yMEpRawdK20E5dzMO0= on the fixed tree). Also added deterministic classification for `nar hash mismatch`, `hash mismatch in fixed-output derivation`, `lock file contains`, and `cannot update locked input` with a negative test proving unrelated `hash` text is not captured, and added a bounded redacted terminal eval-log entry when preflight discovery fails so the drawer no longer ends at `Evaluating nixosConfigurations...`. Verified: `nix build .#checks.x86_64-linux.verified-source-evaluator-parity` exit 0; `cargo test -p cf-server --lib models::evaluate_with_policies` 41 passed / 24 ignored; targeted classification and preflight-log tests 5 passed; rustfmt and `git diff --check` clean. NOT done and reported to the maintainer: expected-system discovery still re-resolves the root flake from Git rather than reusing the materialized NAR-qualified source, but evidence shows that refactor would NOT have prevented this failure because the bad input is a transitive node carried in the source tree's own flake.lock; attempt lineage for commit_id 3262/3263 was not proven because this session has no authorized production database access.
