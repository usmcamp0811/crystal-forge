...
<!-- SECTION:PLAN:BEGIN -->
2026-09-16 scanning/CVE parity and failed-build recovery pass from exact remote `481ae958`: (1) recover the existing task-owned preview using its isolated worktree database, verify process/database/backend/browser identity, and keep `/builds`, `/scanning`, and `/cves` populated and available; (2) first repair build retry end-to-end by reconciling retry lineage with the current derivation uniqueness schema, making requests idempotent/race-safe, rejecting obsolete evaluator-contract targets with actionable same-revision guidance, returning and presenting the resulting attempt identity, fixing bulk partial-failure accounting and selection, and proving normal production claim eligibility; commit and push this outcome independently after focused DB/API/UI checks; (3) align Scanning with the checked-in design using the existing scheduler and scan actions, add bounded server-side list/filter/continuation contracts where needed, truthful lifecycle/findings/error states, schedule persistence, polling that preserves UI state, and a real-evidence drawer without fabricated logs; (4) align Fleet CVE list, detail drawer, and per-environment triage presentation with the current design while preserving exact/legacy/mixed evidence authority, typed assignees, paging, and server-recomputed mutation scope; (5) keep generated CSS and the preview current, run focused isolated PostgreSQL/Rust/WASM/browser checks and inspect reference-comparison screenshots at required themes/viewports; (6) independently review each diff, create separate reviewable commits, push normally, and update task notes without marking TASK-440 complete. No deployment, merge, rebase, force-push, live-queue mutation, migration rewrite, alternate scanner architecture, or unrelated cleanup.

2026-09-16 bounded failed-build retry/requeue outcome from exact HEAD 481ae958: add additive migration 0262 replacing only the global derivation uniqueness index with one-active-execution uniqueness while preserving automatic-child uniqueness; make manual requeue transactionally serialize per derivation, return created/reused typed outcomes, reject evaluator_contract_obsolete with same-revision re-evaluation guidance, and prove normal claim compatibility; add explicit CSRF and truthful typed handler errors; expose only required commit/failure metadata to operator Web UI history; update row/drawer/bulk terminal retry behavior with duplicate-submit prevention, accurate summaries, resulting-attempt visibility, and canonical re-evaluation for obsolete jobs; update focused Rust/PostgreSQL/frontend/browser contracts and run targeted Nix-based formatting/tests. Do not commit, push, deploy, manage preview processes, touch the scanner stash or generated tailwind.css, or modify scanning/CVE pages.

2026-09-16 independent-review correction pass: preserve Webb's active claim/build path and normal production claim SQL. Replace obsolete same-row revival with an immutable queued child allocated under one derivation advisory lock then queue-order lock; retain source status/code/logs/timestamps and allocate lineage plus MAX(attempt_number)+1. Apply the same derivation-first lock order to manual, automatic, and authoritative paths and prove manual/automatic and manual/authoritative serialization with focused PostgreSQL regressions. Make requeue authorization use authenticated roles and source-environment membership with non-disclosing 404, while returning a stable 409 for visible non-terminal and authoritative evaluator-contract-obsolete conflicts. Preserve CSRF and admin-only same-revision re-evaluation. Add a structured Web UI requeue error path so authoritative 409 drives admin guidance even when list metadata is stale or absent. Correct row/drawer/bulk pending state, exact attempt identity/number feedback, conditional Active navigation and filters, status/alert announcements, accessible terminal checkboxes and confirmation modal behavior, and remove unimplemented completed actions. Update focused Rust/browser contracts and migration 0262 documentation only; do not edit migration 0261, scanning/CVE surfaces, evaluator semantics, remote-builder strategy, deployment/stash, or generated Tailwind. Run cargo fmt and targeted Nix-backed checks as feasible; do not commit or push.
<!-- SECTION:PLAN:END -->
