...
Fix the Config Explorer lifecycle regression by using one scope-reset effect that invalidates prior top-level requests and immediately starts one shallow Root observation for an enabled exact revision. Replace hidden retry signals with direct one-request retry handlers. Add focused operation-count and CSS contract regressions. Run only focused Rust tests, touched-file rustfmt, wasm32 Web UI cargo check, git diff --check, and any applicable static CSS assertion before committing and pushing the existing task branch.

2026-09-12 final b428b606 design follow-up from exact clean SHA b428b606bfe64ab3b09a2a40e5d3952f68aff65e: add migration 0258 only for typed POA&M assignees, preserving `owner` as the compatibility/display snapshot and all legacy rows. Persist `owner_kind`, a user UUID protected with deliberate non-destructive historical semantics, and normalized OIDC group-name snapshot without an OIDC mapping FK; enforce mutually exclusive typed shapes and do not infer types from legacy text. Add backward-compatible tagged assignee request/view DTOs, server-authoritative user/group resolution, ambiguous owner+assignee rejection, unassignment, optimistic updates, old/new typed activity snapshots, unchanged owner search/filter behavior, and a bounded Operator/Admin assignee catalog containing only active human users and configured normalized group names. Preserve environment/RBAC authorization as the sole visibility boundary and add falsification coverage proving assignment grants no access. Update the production POA&M create and detail controls to one reusable People/Groups selector with Unassigned, current-user default only when catalog-eligible, historical unavailable-owner retention, loading/error behavior, and lifecycle tooltips; retain existing production CSS that already aligns all `.poam-modal .field` controls. The required CVE authority audit returned NO: current `poam_findings` identity is system plus policy lineage and `FindingObservationSource::CveScan` proves only aggregate require_cve_check counts for a deployed derivation; SystemVulnerability exposes no scan/package occurrence identity, and verification can pass while the intended CVE remains. Therefore do not add the CVE button/modal, do not add migration 0259, and report the dedicated exact-CVE finding contract required. Verify with focused migration and poam_workflows PostgreSQL tests in an isolated repository-owned database, POA&M server/UI tests, SQLX_OFFLINE cf-server tests check, server/Web UI rustfmt, wasm32 Web UI check, git diff check, and the bounded server-regressions check if practical. Do not run heavyweight Web UI/VM/flake checks, merge/rebase dev, resolve MR conflicts, deploy, or merge !323. Commit assignees only as `TASK-440: Add typed POA&M assignees`, then fetch and push normally only if the remote still contains b428b606 plus this commit.

2026-09-12 server/database-only typed POA&M assignee execution from clean exact SHA b428b606bfe64ab3b09a2a40e5d3952f68aff65e: add only migration 0258 with nullable typed columns, restrictive user FK, no OIDC mapping FK, and exact legacy/user/group shape constraint; add documented request/view/catalog DTOs; resolve eligible human users and normalized configured OIDC groups in the POA&M service; preserve owner snapshot search, legacy compatibility, visibility boundaries, revision semantics, and typed old/new audit snapshots; add the assignee catalog route before the dynamic POA&M route; extend every summary query and focused poam_workflows PostgreSQL/HTTP tests. Modify no Web UI files and add no CVE/finding behavior. Verify affected server rustfmt, targeted POA&M tests where practical, SQLX_OFFLINE=true cargo check --offline -p cf-server --tests, migration application, and git diff --check. Do not commit, push, merge, or rebase.

2026-09-13 final queued finish sequence: first finish, focused-test, commit, and push the current POA&M milestone/activity detail parity slice. Correct Save-path focus restoration only after the real mutation reconciles and re-enables the title trigger; keep Cancel immediate; add a focused browser contract for Save-path focus and use milestone-specific removal labels. Then implement and independently verify/commit/push the additive exact-CVE POA&M backend and UI contract. From that exact clean synchronized head, record STARTING_SHA and perform the authorized topbar notification presentation/interaction parity pass against Shell.jsx, styles.css, and alerts-and-notifications.md without migrations, backend redesign, or event-semantic changes. Preserve account-scoped durable state, authorization, stale-response guards, read/dismiss distinction, pagination, routing, and focus management; add focused Web UI/static/browser contracts; run fast verification only; commit as `TASK-440: Match notification center design` and push normally after a remote-head guard. Only after all three feature slices are clean and synchronized, audit migration numbering and merge current origin/dev without rebase, resolve conflicts semantically, run focused post-merge and exact-head CI verification, update MR/task notes, use only the established non-production deployment mechanism for smoke, verify AC #24/#27, and move to Review only when fully proven. Do not merge MR !323.

2026-09-13 exact-CVE authority implementation from clean synchronized SHA 1204efdc8cf7e25ed01a634832456df0262c08af: keep all policy POA&M tables, fields, DTO meanings, routes, and verification behavior intact. Add migration 0259 with `cve_scans.evidence_schema_version`, immutable scan-scoped `cve_scan_vulnerability_observations`, and the required dedicated `poam_cve_findings`, `poam_cve_finding_links`, and `poam_cve_verification_items`. Stable finding identity is `(system_id, canonical_cve_id, canonical_package_name)`; installed version, package derivation, scan, store path, whitelist/justification, and timestamps are evidence only. The successful Vulnix writer must canonicalize CVE IDs and package `pname`, snapshot each exact occurrence in the same transaction as scan completion, and set evidence schema version only for fully persisted successful output; legacy scans must never be reconstructed from mutable `package_vulnerabilities` and cannot produce exact-CVE PASS. Protect completed scan observations, stable finding identity, retired links, and sealed verification evidence from mutation. Add rolling-compatible `cve_findings` and `cve_items` response fields with defaults while preserving `findings` and `items` meanings. Add dedicated create/link/unlink/relationship routes and server-issued exact context; preserve environment visibility, non-disclosure, Operator/Admin mutation policy, optimistic revisions, one-active-remediation uniqueness, activity, POA&M notifications, and deterministic advisory-lock ordering. Exact verification must resolve the current system store path to its exact NixOS derivation, select the latest completed schema-capable scan for that derivation, FAIL while the exact canonical CVE/package occurrence remains, and PASS only when that exact occurrence is absent; missing/legacy/no exact scan, changed deployment, inconsistent evidence, whitelist, or justification must not PASS, and only PASS permits closure. Extend close/reopen and database invariants across the union of policy and exact-CVE links without permitting mixed finding kinds. UI: extend real System CVE DTO rows with server-issued remediation relationships; provide Create POA&M/active reference using the existing typed assignee and lifecycle controls; retain justification as an independent action; show linked vulnerabilities and exact verification evidence separately in POA&M detail; preserve package/version context and authorization. Add focused migration, scan writer, PostgreSQL workflow/concurrency/visibility/legacy compatibility, server model/query, Web UI grouping/component/API, and browser/static contracts. Verify targeted server/UI tests, isolated PostgreSQL regressions, SQLX offline check, server-regressions if practical, touched rustfmt, wasm check, Node syntax, and diff checks. Do not merge dev, run heavyweight browser/flake checks locally, or start notification parity until this slice is committed, pushed, clean, and synchronized.

2026-09-13 hostile-review remediation for the uncommitted exact-CVE backend slice: enforce current-environment filtering for historical exact items; preserve clean-scan remediation while failing present justified/whitelisted occurrences and fabricated PASS; bind current evidence through the retained exact system generation snapshot rather than store-path-only derivation selection; make scan occurrences self-contained from mutable package derivations; admit and audit exact link/unlink activity; serialize fleet justifications and exact-CVE lifecycle with a global canonical-CVE lock while preserving system/finding order; add bounded server-issued exact remediation context to SystemVulnerability and cap exact detail history; persist observed package version in verification evidence; enforce schema-1 completion timestamps and deterministic selection; retain only scans referenced by sealed verification evidence while allowing normal parent cascade cleanup; document authority, identity, outcomes, locks, retention, rolling fields, and negative guarantees; add focused PostgreSQL/HTTP/concurrency/deletion regressions. Run focused non-skipping database PostgreSQL tests, SQLX offline check, rustfmt, and diff checks. Do not modify Web UI files, commit, or push.

2026-09-13 full-integration backend P1 correction for the uncommitted exact-CVE slice: replace `/systems/:id/cves` hostname-view rows with one bounded query over the exact current deployed generation binding and its latest completed evidence-schema-1 scan; join immutable observations to CVE metadata and current system/fleet justification data required by `SystemVulnerability`; return no inferred rows when exact authority is unavailable. Remove package-version equality from row relationship identity so matching uses system plus canonical CVE plus canonical package while the exact observation returns version evidence; preserve deterministic conflicting-occurrence selection and the 1000-row bound. Add PostgreSQL regressions for excluding a newer undeployed derivation scan, including the exact deployed scan, and hydrating context for overlong observed versions. Run focused non-skipping database tests, `SQLX_OFFLINE=true cargo check --offline -p cf-server --tests`, server rustfmt, and staged/unstaged diff checks. Do not modify Web UI files, commit, or push.

2026-09-13 redesign realignment at exact local/remote HEAD 9725b4efc274346cd64e4408e40a776b3a0426e0: preserved the full staged pre-realignment implementation at `/tmp/task440-exact-cve-pre-9725-staged.patch`; no unstaged or untracked files existed. Retain the sound immutable schema-1 scan occurrence, exact retained-generation authority, stable `(system, CVE, canonical package)` finding identity, reference-bounded retention, multi-finding verification/close, typed assignee, and additive DTO foundations. Add normalized authoritative environment dispositions and one atomic fleet triage domain/API that accepts environment choices but recomputes affected systems, deployed generations, exact occurrences, authorization, and conflicts server-side. Mixed OPEN/ACCEPTED/SCHEDULED state must be durable. ACCEPTED records justification, authenticated actor, timestamp, and optional review date without creating or passing remediation. SCHEDULED creates or reuses one POA&M covering every exact subject in all selected environments; partial pre-existing management returns a typed conflict and never steals links. Revocation/change retires only the selected environment's links/disposition transactionally and cannot invalidate unrelated POA&M subjects. Use one shared domain service for fleet triage and the System Detail narrow entry. Enforce the authoritative lock order globally: canonical CVE keys, then all system sentinels, then all policy keys, then all exact keys, with lexical ordering at each level; align scan publication, disposition writes, link lifecycle, verification, close, and deployment/state fencing. Add fleet/environment rollup DTOs and the 9725 CVE drawer/modal UI with per-environment choices, one typed-assignee schedule section, one required-justification acceptance section, severity target defaults, and vulnerability-specific standard milestones. Add the required mixed-disposition, multi-environment one-POA&M, forged-scope, duplicate conflict, authorization/non-grant, retained-lineage failure, immutable evidence retention, multi-system 9+1 failure/missing, revocation isolation, exact System CVE read, and deterministic concurrency regressions. Keep ATO/evidence-package export out of scope. Adapt current staged work before any exact-CVE commit or push; preferred commits are exact finding/evidence foundation followed by fleet triage workflow. After both are focused-tested, reviewed, pushed, clean, and synchronized, proceed to notification-center parity before merging dev.
