...
2026-09-15 bounded System CVE paging Web UI slice implemented in the existing dedicated TASK-440 worktree. The client now requests fixed 100-row pages with URL-encoded opaque cursors. System Detail appends and deduplicates findings by canonical CVE/package identity, suppresses stale responses, restarts safely on inventory-version/source changes, preserves loaded rows across continuation errors, and displays authoritative filtered totals and exact/legacy/no-scan/clean metadata. Added reducer/client/DTO tests and browser workflow `12ha-system-detail-cve-inventory-fallbacks` with a 1,315-finding mock, cursor overlap deduplication, exact/legacy precedence, clean, and no-scan states. Verification passed: full Web UI unit suite (415 passed, 0 failed, 1 ignored); focused CVE state tests (8 passed); offline WASM cargo check; scoped rustfmt for the four changed Rust UI files; JavaScript syntax and coverage-manifest parsing; `git diff --check`; and `nix develop -c web-ui-test 12ha-system-detail-cve-inventory-fallbacks` after the final UI adjustment (1/1 passed, dark and light captures). The live preview completed its final rebuild and remains available at port 8080 with the server on port 3445. The authoritative `12h-system-detail-cves-grouped-justification` workflow remains unrun because the live-preview runner identifies it as VM-only and the maintainer prohibited broad/costly Web UI VM execution. Package-wide `cargo fmt --check` is also blocked by pre-existing formatting drift in `components/flake/flake_timeline.rs` and `components/onboarding/coach_panel.rs`; neither file belongs to this slice or was modified. No commit, push, deployment, MR, or task-state transition was performed.
<!-- SECTION:NOTES:END -->
