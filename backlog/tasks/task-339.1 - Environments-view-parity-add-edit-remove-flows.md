...
2026-09-20 stale-input diagnosis correction and replacement run result: Proved the earlier stale-Nix-input theory was WRONG. `nix flake metadata --json .` resolved a store source path whose `checks/web-ui/tests/integration-test.js` sha256 matched the worktree file exactly, and it already contained the corrected regex selector from the prior edit. The real root cause of the original 14a failure was a Playwright strict-mode locator ambiguity, not stale build input or CSS text-transform: `dialog.getByText(/explicit cache destinations/i)` matched two elements simultaneously (the Binary cache SectionHead description "Choose explicit cache destinations for this environment..." and the field label "Explicit cache destinations"), because Playwright's getByText performs case-insensitive substring matching by default. Fixed by switching that one locator to an exact match: `dialog.getByText("Explicit cache destinations", { exact: true })`, which is unique in the dialog. Verified via `nix develop -c node --check` and `git diff --check`, then reconfirmed the Nix path-flake source (new store path after the edit) contained the exact-match fix with a second matching sha256 comparison before running the browser check again.

Ran the one authorized replacement build: `CF_UI_TEST_STEPS="14a-environments-add-modal" nix build --impure "path:.#checks.x86_64-linux.web-ui" --no-link -L`. Result: still failed (exit 1, 0/1 screenshots), but progressed past the cache-section assertion this time, proving the exact-match fix worked. The failure moved to the next assertion, `Expected policy enforcement section` on `dialog.getByText("Gate policies")`. Static inspection of `environment_form_modal.rs` shows the identical defect class: the Policy enforcement `SectionHead` description ("Pick gate policies or require compliance bundles...") and the `Gate policies` field label both contain the case-insensitive substring "gate policies", so the same-style locator is ambiguous there too. The rendered page body in the failure diagnostic proves the Policy enforcement section, the `required-agent` gate policy chip, and the count copy ("1 gate policy assignments apply before deployment.") are all present and correct — this is a second instance of the same test-harness selector defect, not a product defect. No further VM run was made; the one authorized replacement run is exhausted. Awaiting explicit approval before fixing this second locator and requesting another authorized run.
<!-- SECTION:NOTES:END -->
