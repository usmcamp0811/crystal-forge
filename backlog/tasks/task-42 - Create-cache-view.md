...
Updated web-ui screenshot check to capture cache Add Destination modal states for all cache types (Nix, Http, S3, Attic) in addition to the cache page view; reran `nix build .#checks.x86_64-linux.web-ui` successfully.

Updated MR !155 UI section to remove non-cache screenshots and include only cache page and cache modal screenshots generated by the successful web-ui check run.

Commit pushed: c09f37cd

Addressed review blockers in follow-up commit c39600fb: removed stray cache handler backup files, added merged-shape validation for cache PUT updates, added cache existence check for environment assignment (404 on missing cache), added PUT to dev CORS allow list, and aligned cache cancel semantics wording.

Updated cache credential requirements: Attic token is required and S3 region/access key/secret/endpoint are required in backend validation and Add/Edit cache modal validation/UI labels.

Verification run: `nix develop -c bash -c "cd packages/default && SQLX_OFFLINE=true cargo check"` (pass), `nix develop -c bash -c "cd packages/web-ui && cargo check"` (pass), `nix develop -c bash -c "cd packages/default && cargo test --lib models::cache_destination::tests::test_validate_attic"` (pass).
<!-- SECTION:NOTES:END -->
