...
2026-08-05: Diagnosed deployed-server startup failure: backfill_pending_digests only selected mutable publication states, but final verification counted immutable pending rows. Added policy JSON/TOML import endpoint with draft/disabled/untrusted defaults and legacy single-expression normalization. Added migration 0205 permitting only pending->computed digest repair on immutable policy/bundle versions, while preserving normal immutability. Updated backfill queries to process all pending versions. Verified with Nix dev shell: cargo fmt --all -- --check, SQLX_OFFLINE=true cargo check -p cf-server, git diff --check, policy import unit tests (2 passed), resolver unit tests (18 passed). Changes are local and not pushed.

2026-08-05: Committed and pushed startup digest repair as 50296b7d (fix(compliance): repair pending immutable digests) to TASK-412-cf-xccdf-interchange. Remote branch is updated; unrelated deployment/mod.rs remains unstaged.

2026-08-05: Continued Option A implementation locally: added admin-only POST /api/v1/policies/interchange/preview with SHA-256 preview metadata and policy summaries; import now reuses the same parser. Added JSON/TOML parser coverage. Fixed new-lineage import handling so the legacy INSERT trigger's auto-created draft is replaced by the portable imported version ID and current-draft pointer. Verification: Nix dev shell cargo fmt check, cf-server cargo check previously passed after endpoint changes, and policy-focused tests now 3 passed / 2 ignored. Changes are currently uncommitted; unrelated deployment/mod.rs remains unstaged.

2026-08-05 continuation: inspected policy interchange backend/UI contract. Policy preview returns source_sha256, policy_count, policy rows with implementation_state and semantic_digest; validation returns typed JSON 422/409 bodies, while the shared client currently strips those bodies. Implementing a small policy-only diagnostic adapter that preserves raw interchange error bodies, maps known HTTP statuses, keeps preview on non-digest conflicts, and clears it on digest mismatch. No backend DTO change is needed.

2026-08-05 continuation completed and pushed: 3af02290 adds policy-only diagnostic normalization using the existing 422/409/403/413/415/500 response shapes; 067ff487 clears diagnostics on preview retry and successful commit; 7ce233d3 hardens export filename separator collapsing and adds its test. Verification passed: web-ui diagnostic tests (4), filename test (1), server parser test (1), SQLX offline server cargo check, web-ui cargo check, git diff --check, and xccdf schema Nix check. Web UI Nix VM check timed out after 900000 ms while building the integration VM; full flake check was not run because the required web-ui gate did not complete. MR head matches remote at 7ce233d3; deployment/mod.rs remains the only unstaged unrelated change.

2026-08-06: Continued P1 fixes on existing TASK-412 worktree. Pushed fe57ab99 (deployment gates now use resolver effective_config), 542ae256 (Nix evaluation resolves system effective policies), 7820068b (combined resolver digest includes bundle/addition/direct sources, modes, and effective configs), 5aad951d (rollups/evidence use resolver output for assigned bundle), c269de33 (assignment exclusion/addition editor and effective-set preview in Compliance UI), 4c9ad700 (nested XCCDF export group regression test), and 0d9eab2a (deterministic DISA-style nested ZIP package processing test). Focused server/UI compilation and targeted tests passed. Real DB-backed preview/import/publish/assign/export with an official DISA artifact remains unavailable locally because DATABASE_URL and official artifact are not present; existing ignored acceptance test remains the authoritative external check.

Verification update: server and web-ui cargo checks pass; targeted resolver/digest (34 tests), nested export-group, and DISA-style ZIP tests pass. `nix build .#checks.x86_64-linux.web-ui --no-link` was attempted with a 20-minute timeout and reached the VM integration derivations before timing out; it was not reported as passed. Full DB-backed official DISA workflow remains unverified because DATABASE_URL and the official ZIP are unavailable locally.

2026-08-06: Reproduced the reported import failure in the upload contract. The WASM client appended the JSON import plan as a Blob with filename plan.json, so Axum classified it as a second file and rejected it as an unexpected file field. Fixed the client to send plan as a normal multipart text field and made the server accept the plan field regardless of an incidental filename. Added the missing design actions: Refine N policies and Skip & create all; completing Refine now performs the real atomic import instead of only changing the modal to Done. V1R2 artifact could not be verified because /mnt/data/U_Anduril_NixOS_V1R2_STIG.zip is not present in this environment. Verification: SQLX_OFFLINE=true nix develop -c cargo check --manifest-path packages/default/crates/cf-server/Cargo.toml passed; SQLX_OFFLINE=true nix develop -c cargo test --manifest-path packages/default/crates/cf-server/Cargo.toml xccdf --lib passed (204 passed, 2 ignored). Web UI cargo check passed earlier; repository-wide cargo fmt --all --check is currently blocked by pre-existing formatting differences across unrelated files.

2026-08-06: Committed and pushed as 1f20343e (fix(compliance): repair XCCDF import workflow). Remote branch now points to 1f20343e. The user-provided untracked V1R1 ZIP remains untouched.

2026-08-07: Continued on branch TASK-412-cf-xccdf-interchange. Added editable typed assertion and evidence editors to the structured Refine flow, added a final Import Review step before commit, and exposed current policy version IDs so foreign rules can be mapped to existing policy versions. UI checks passed: `nix develop --command cargo check` and `nix develop --command cargo test refine_policy` (2 passed). Server `SQLX_OFFLINE=true nix develop --command cargo check -p cf-server` passed; normal server check was attempted but could not connect to PostgreSQL, producing SQLx connection-refused errors. Pushed commits 667f8104, f721b0f6, and 0662836d to MR !313. The old unreachable legacy Refine block remains to be removed in a later focused cleanup.
...
Implemented the assignment UX pass in `packages/web-ui/src/views/compliance.rs`: environment names and searchable system hostnames are selectable; baseline exclusions and additions use policy-name checkboxes while retaining exact version UUID payloads; selected bundle revision/state and non-current warning are shown; assignment creation is gated on a successful preview of the exact request. `nix develop --command cargo check --manifest-path packages/web-ui/Cargo.toml --target wasm32-unknown-unknown` passed. The Nix web-ui integration build reached source compilation but exceeded the command timeout; no final result.

Implemented focused effective-assignment XCCDF export in the dedicated task worktree. Added GET /api/v1/compliance/assignments/:assignment_id/xccdf, reusing the authoritative assignment resolver and typed XML writer; baseline bundle export remains unchanged. Assignment export deterministically orders resolved policy versions, includes overlay additions/exclusions, applies resolver-produced effective configs, and returns structured conflict/validation errors. No migrations. Verification: SQLX_OFFLINE=true nix develop --command cargo check --manifest-path packages/default/crates/cf-server/Cargo.toml passed. Normal nix develop cargo check could not validate SQLx queries because PostgreSQL connection was refused. cargo fmt --manifest-path ... --check reports existing formatting drift across the dirty worktree; no formatter rewrite was applied. git diff --check passed.
