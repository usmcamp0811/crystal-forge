...
with lib; rec {
  ...
  in {
    options.campground.stig.${name} = with types; {
      enable =
        lib.campground.mkBoolOpt config.campground.stig.enable
        "Enable/Disable ${name}";
      justification =
    ...
    config = mkMerge [
...

      {
        ...
        assertions = [
          {
            assertion =
              (!cfg.enable && config.campground.stig.enable == true)
              -> (cfg.justification != []);
            message = "You must provide at least one justification if config.campground.stig.${name} is disabled.";
          }
        ];
      }
    ];
