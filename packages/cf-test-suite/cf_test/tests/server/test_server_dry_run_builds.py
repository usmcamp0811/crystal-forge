        ...
        if derivation_status:
            ...
            if status_id == 5:
                ...
                if _is_enospc(error_message or ""):
                    ...
                    server.log(
                        "⚠️ Dry-run failed due to test flake structure issues. Treating as environmental; skipping test."
                    )
                    pytest.skip(
                        "Dry-run failed due to test flake structure issues in the VM test environment."
                    )
                else:
                    server.log(
