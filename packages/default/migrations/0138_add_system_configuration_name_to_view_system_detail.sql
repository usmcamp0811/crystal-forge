-- Restore `system_configuration_name` to `view_system_detail`.
--
-- Migrations 0135 (reachability) and 0136 (fqdn) each recreated the full
-- `view_system_detail` definition but dropped the `system_configuration_name`
-- projection. The API/UI `SystemDetail` model still expects it, and the query
-- layer had to compensate by joining `systems` back in. This migration restores
-- the column on the view itself so direct/view consumers stay consistent.
--
-- IMPORTANT: this is a NEW migration. Prior migrations (incl. 0135/0136) are
-- already applied on deployed databases and MUST NOT be edited.
--
-- The definition below mirrors 0136 exactly, only APPENDING
-- `s.system_configuration_name` as the final projected column. `CREATE OR
-- REPLACE VIEW` forbids renaming/reordering existing columns, so the new field
-- must be added at the end of the SELECT list rather than inserted mid-list.
