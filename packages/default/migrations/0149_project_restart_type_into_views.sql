-- Project last_restart_type and last_restart_at into view_system_detail.
--
-- Migration 0148 added the columns to the systems table. This migration appends
-- them to view_system_detail so the API and UI can surface restart classification
-- without an extra JOIN.
--
-- view_system_list is intentionally NOT updated: the list view is used for cards
-- and tables where restart classification is not displayed. Keeping it out of the
-- list view avoids widening the query that runs for every page load.
--
-- IMPORTANT: CREATE OR REPLACE VIEW forbids removing or reordering existing
-- columns. The new columns are appended after boot_id (the current last column).
...
SELECT
    ...
    s.is_active,
    s.deployment_policy,
    -- Fixed fleet-health thresholds (unchanged from migration 0147).
    CASE
    ...
    s.boot_id,
    -- Restart classification (added by migration 0148).
    s.last_restart_type,
    s.last_restart_at
FROM systems s
