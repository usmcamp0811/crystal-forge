...
SELECT
    ...
    -- Combined Status for quick overview
    CASE WHEN lss.hostname IS NULL THEN
        'never_seen'
    WHEN (lhph.heartbeat_timestamp IS NULL
    ...
    END AS drift_hours
FROM
    -- Start with all registered systems to ensure we see never_seen systems
    system_registered_check src
    -- Left join to get system state info (if system has ever reported)
    LEFT JOIN latest_system_states lss ON src.hostname = lss.hostname
    -- Left join to get heartbeat info
    LEFT JOIN latest_heartbeats_per_hostname lhph ON src.hostname = lhph.hostname
    -- Left join to get latest flake commit info
