-- Migration: Update all views affected by system_states schema change
-- from derivation_path to store_path
--
-- Affected views (drop order - reverse dependency):
-- 1. view_config_timeline (depends on view_system_deployment_status)
-- 2. view_system_deployment_status
-- Then recreate in correct order
DROP VIEW IF EXISTS public.view_config_timeline CASCADE;

DROP VIEW IF EXISTS view_system_deployment_status CASCADE;

-- ============================================================================
-- Recreate view_system_deployment_status with store_path
-- ============================================================================
...
-- ============================================================================
-- Recreate view_config_timeline with updated column reference
-- ============================================================================
CREATE OR REPLACE VIEW public.view_config_timeline AS
WITH current_counts AS (
    SELECT
        s.current_commit_hash,
        s.flake_name,
        COUNT(*) FILTER (WHERE s.current_store_path IS NOT NULL
            AND s.deployment_status IN ('up_to_date', 'behind', 'ahead')) AS systems_deployed_now
    FROM
        public.view_system_deployment_status AS s
    GROUP BY
        s.current_commit_hash,
        s.flake_name
)
SELECT
    v.commit_timestamp AS time,
    CONCAT(COALESCE(cc.systems_deployed_now, 0), ' deployed (', v.short_hash, ')§', ROW_NUMBER() OVER (PARTITION BY v.flake_name ORDER BY v.commit_timestamp DESC) - 1) AS "Config",
    v.flake_name
FROM
    public.view_commit_deployment_timeline AS v
    LEFT JOIN current_counts AS cc ON cc.current_commit_hash = v.git_commit_hash
        AND cc.flake_name = v.flake_name
    ORDER BY
        v.commit_timestamp DESC;

COMMENT ON VIEW public.view_config_timeline IS 'Shows commit evaluation timeline in a format to be easily parsed into the config timeline in Grafana.';
