...
impl BuildConfig {
    ...
    pub fn apply_to_legacy_nix_store_realise_command(&self, cmd: &mut tokio::process::Command) {
        self.apply_common_nix_options(cmd);
...
mod tests {
    ...
    fn test_legacy_realise_command_maps_offline_to_no_substitute() {
        for (offline, use_substitutes, expected_no_substitute) in [
            ...
            assert!(
                args.windows(3)
                    .any(|part| part[0] == "--option" && part[1] == "max-silent-time")
            );
            assert!(
                args.windows(3)
                    .any(|part| part[0] == "--option" && part[1] == "timeout")
            );
            assert!(
                args.windows(3)
                    .any(|part| part[0] == "--option" && part[1] == "sandbox")
            );
        }
    }
}
