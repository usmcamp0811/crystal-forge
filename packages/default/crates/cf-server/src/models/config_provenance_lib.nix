...
let
  ...
  buildRawDefinitionsByOption = { options, normalizedModules }:
    let
    ...
    in map (key: {
      option_key = key;
      path = (builtins.head groupedDefinitions."${key}").path;
      definitions = groupedDefinitions."${key}";
    })
      (builtins.attrNames groupedDefinitions);

  buildDefinitionsByOption = { options, normalizedModules }:
    map (option: decorate option.option_key option.definitions)
      (buildRawDefinitionsByOption { inherit options normalizedModules; });

  rawDefinitionsByOption = buildRawDefinitionsByOption {
    options = configuration.options;
    normalizedModules = replayGraph.modules;
  };

  definitionsByOption = map (option: decorate option.option_key option.definitions)
    rawDefinitionsByOption;

  canonicalMetadata = {
    provenanceAdapterVersion = provenanceAdapterVersion;
    targetLibVersion = configuration.pkgs.lib.version or null;
    targetModuleSystemPath = toString (configuration.pkgs.path or "");
    definitionsByOption = map (option: {
      ...
      }) (lib.sort (a: b: a.ordinal < b.ordinal) option.definitions);
    }) definitionsByOption;
  };

  provenanceDigest = builtins.hashString "sha256" (builtins.toJSON canonicalMetadata);
...
in {
  inherit canonicalMetadata definitionsByOption rawDefinitionsByOption provenanceDigest;
  provenance = result // (if result.supported or false then {
    inherit provenanceDigest;
  } else { });
}
