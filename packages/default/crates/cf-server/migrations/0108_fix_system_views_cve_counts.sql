-- Restore real CVE counts in system views.
--
-- view_system_list and view_system_detail currently hardcode CVE counts to 0,
-- which causes Systems UI/API to always show zero vulnerabilities even when
-- view_system_vulnerabilities has data.
...
cve_counts AS (
    SELECT
        hostname,
        COUNT(*) FILTER (WHERE severity = 'CRITICAL')::integer AS critical_cve_count,
        COUNT(*) FILTER (WHERE severity = 'HIGH')::integer AS high_cve_count,
        COUNT(*) FILTER (WHERE severity = 'MEDIUM')::integer AS medium_cve_count,
        COUNT(*) FILTER (WHERE severity = 'LOW')::integer AS low_cve_count
