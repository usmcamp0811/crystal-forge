-- A newer failed derivation can reuse the observed path of an older deployable
-- build. Bind running identity to the eligible scoped artifact first, without
-- losing historical path context when no eligible mapping exists. Keep the
-- eleven-column contract used by the system list and detail views.
...
WITH latest_system_states AS (
    ...
    -- A path may have multiple historical records. Prefer an exactly
    -- cache-published, eligible artifact of this registered configuration;
    -- otherwise retain the historical scoped/NixOS/newest ordering.
    LEFT JOIN LATERAL (
        ...
        ORDER BY (c.flake_id = s.flake_id
                  AND d.derivation_type = 'nixos'
                  AND d.derivation_name = COALESCE(
                      NULLIF(BTRIM(s.system_configuration_name), ''), s.hostname
                  )
                  AND d.store_path = lss.store_path
