...
--
-- Also fixes view_environment_rollups (which had its own copy of the CTE).
--
-- NOTE: Must DROP views first because PostgreSQL cannot replace views when column
-- order changes. CASCADE will drop dependent views.
...
COMMENT ON VIEW public.view_system_detail IS
    'Detailed per-system health and metadata for API and UI consumption. Added by migration 0152.';
...
COMMENT ON VIEW public.view_system_list IS
    'Per-system health summary for API list endpoints and UI cards. Added by migration 0152.';

-- ────────────────────────────────────────────────────────────────────────────
-- Recreate view_environment_rollups (originally from migration 0140)
-- ────────────────────────────────────────────────────────────────────────────
...
COMMENT ON VIEW public.view_environment_rollups IS
    'Per-environment active-system health breakdown, critical+high CVE totals, and flake names. Fixed by migration 0152 to include system_states timestamps.';
