-- Restore fixed fleet-health thresholds in view_system_detail and view_system_list.
--
-- Migration 0146 introduced per-system heartbeat_interval_secs and boot_id columns,
-- but its CREATE OR REPLACE VIEW bodies accidentally replaced the fixed thresholds:
--   warning  → 15 minutes
--   critical → 1 hour
--   offline  → 4 hours
-- with interval-derived multipliers (e.g., interval * 3/4/6). At the default 600-second
-- interval this shortened offline detection from 4 hours to ~60 minutes, contradicting
-- TASK-378's explicit non-goal of leaving these thresholds unchanged.
--
-- This migration restores the fixed thresholds while preserving every column added in
-- 0146 (heartbeat_interval_secs and boot_id) so no downstream query breaks.
--
-- The views are recreated via CREATE OR REPLACE VIEW which is safe: existing column
-- names and ordinals are kept; new columns are appended.
--
-- IMPORTANT: 0146 is already applied on deployed databases and MUST NOT be edited.
-- This migration is the corrective forward-only patch.
...
SELECT
    ...
    s.is_active,
    s.deployment_policy,
    -- Fixed fleet-health thresholds (restored from migration 0138).
    -- These are intentionally NOT derived from heartbeat_interval_secs; a separate
    -- policy redesign task must be created if interval-relative thresholds are wanted.
    CASE
...
-- Restore fixed thresholds in view_system_list as well.
