-- 0287 was applied to the isolated task database. Source archival is not an
-- artifact deployment gate: a built, cache-published output can remain usable
-- after its commit's source has been archived. Preserve all dependent view
-- columns, ordering, and types while aligning the effective configuration.
...
WITH latest_system_states AS (
    ...
    -- A reported store path can match several derivations. Prefer this host's
    -- registered flake/configuration and select at most one mapping.
    LEFT JOIN LATERAL (
    ...
    LEFT JOIN LATERAL (
        -- Count only distinct commits with a deployable artifact for this host.
        -- A different eligible artifact at an equal timestamp can be behind
        -- with zero newer commit IDs, so descriptions never state that count.
