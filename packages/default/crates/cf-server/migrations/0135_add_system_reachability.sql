-- Persist how Crystal Forge should expect to reach a system.
-- "direct" means the server/operator can connect to the host address directly.
-- "pull" means the agent is pull-only / behind NAT or firewall and may require a bastion.

ALTER TABLE public.systems
    ADD COLUMN IF NOT EXISTS reachability text NOT NULL DEFAULT 'direct';

ALTER TABLE public.systems
    DROP CONSTRAINT IF EXISTS systems_reachability_check;

ALTER TABLE public.systems
    ADD CONSTRAINT systems_reachability_check
    CHECK (reachability IN ('direct', 'pull'));

COMMENT ON COLUMN public.systems.reachability IS
    'How Crystal Forge should expect to reach this host: direct or pull.';
