-- Scope cve_counts CTEs in view_system_list and view_system_detail to active
-- systems only.
--
-- Migration 0111 replaced COUNT(*) with COUNT(DISTINCT cve_id) in the
-- cve_counts CTE of both views, but the CTE still reads directly from
-- view_system_vulnerabilities grouped by hostname with no join back to
-- systems. This means CVE counts could include hostnames that exist in
-- vulnerability rows but are no longer active systems, causing the per-system
-- severity chips and detail header to drift from the rest of the dashboard.
--
-- Fix: add JOIN systems s ON s.hostname = ... WHERE s.is_active = TRUE
-- to the cve_counts CTE in both views, matching the filter already applied
-- in the dashboard summary queries.
--
-- Uses CREATE OR REPLACE VIEW — idempotent and forward-safe.
-- Migration 0111 is not modified.
