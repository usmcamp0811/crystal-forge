-- Persist an operator-managed fully-qualified domain name for a system.
-- This is separate from hostname/environment so Edit System can save the FQDN
-- field instead of treating it as local-only UI state.

ALTER TABLE public.systems
    ADD COLUMN IF NOT EXISTS fqdn text;

COMMENT ON COLUMN public.systems.fqdn IS
    'Optional operator-managed FQDN for this system. When null, UI may derive one from hostname and environment.';
