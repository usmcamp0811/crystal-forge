...
async fn execute_build_job(
    ...
    let cache_config = derivation_payload
        .cache_push
        .as_ref()
        .map(|cache_push| builder_cache_to_config(cache_push, &local_cache_config))
    ...
    let metrics_task = if let Some(ref ws) = ws_shared {
        ...
        Some(tokio::spawn(async move {
            ...
            loop {
                ...
                if let Err(e) = cf_builder::builder::api_client::BuilderApiClient::send_metrics(
                    &mut *ws,
                    cpu_percent,
                    ram_used_mb,
                    ram_total_mb,
...
) -> std::result::Result<(), cf_builder::builder::api_client::DeltaError> {
    use cf_builder::builder::api_client::DeltaError;
...
mod tests {
    ...
    fn cleanup_candidate_is_limited_to_configured_worktree_root() {
        let job_id = uuid::Uuid::parse_str("11111111-2222-3333-4444-555555555555")
            .expect("uuid should parse");
        let payload = cf_protocol::builder::BuildJobDerivation {
            id: 1,
    ...
    fn path_materialization_failure_phase_serializes_correctly() {
        use cf_protocol::builder::BuildFailurePhase;
        assert_eq!(
