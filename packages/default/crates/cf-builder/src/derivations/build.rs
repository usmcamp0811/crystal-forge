...
use crate::gc_root::get_gc_root_path;
use cf_config::config::BuildConfig;
use cf_config::config::CacheConfig;
use anyhow::Context;
...
// Builder-side: resolve .drv path to output store path (no DB)
impl Derivation {
    pub async fn resolve_drv_to_store_path(drv_path: &str) -> anyhow::Result<String> {
        if !drv_path.ends_with(".drv") {
