...
impl<'de> serde::Deserialize<'de> for WatchedFlake {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        struct Helper {
            ...
            auto_poll: bool,
            #[serde(default = "default_initial_commit_depth")]
            initial_commit_depth: usize,
        }

        let helper = Helper::deserialize(deserializer)?;
        let branch = parse_branch_from_url(&helper.repo_url);

        Ok(WatchedFlake {
            name: helper.name,
            repo_url: helper.repo_url,
            auto_poll: helper.auto_poll,
            initial_commit_depth: helper.initial_commit_depth,
            branch,
        })
    }
}
