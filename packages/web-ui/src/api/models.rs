...
pub struct SystemDetail {
    ...
    /// Read-only; the server derives it from `systems.public_key`. `None` when
    /// the server could not parse the stored key.
    #[serde(default)]
...
pub enum AuditAction {
    ...
    /// A system's agent public key was replaced (TASK-435).
    SystemKeyRotated,
    SessionInvalidated,
    CveScanRequested,
    BuilderRotateKey,
