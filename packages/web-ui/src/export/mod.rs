...
pub fn build_oscal(p: &ExportPayload<'_>) -> String {
    ...
    // resolves to a parseable SSP within the back-matter.
    let ssp_uuid = uuid::Uuid::new_v4().to_string();
    let ssp_json = json!({
    ...
    let ssp_json_str = serde_json::to_string_pretty(&ssp_json).unwrap_or_default();
                ...
                "resources": [
                    {
                        "uuid": ap_uuid,
                        "title": format!("Assessment Plan for {}", p.bundle.name),
                        "description": "Embedded minimal OSCAL Assessment Plan governing this assessment. Contains import-ssp referencing the embedded SSP, reviewed-controls, and local objective definitions.",
                        "base64": {
                    ...
                    },
                    {
                        "uuid": ssp_uuid,
                        "title": format!("System Security Plan for {}", p.bundle.name),
                        "description": "Embedded minimal OSCAL System Security Plan referenced by the Assessment Plan (import-ssp). Identifies assessed systems and their components.",
                        "base64": {
                            "filename": format!("{}-system-security-plan.json", slugify_for_filename(&p.bundle.name)),
                            "media-type": "application/oscal+json",
                            "value": ssp_base64,
                        }
                    }
