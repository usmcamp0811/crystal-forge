...
@pytest.mark.parametrize(
    "scenario_config", DEPLOYMENT_TIMELINE_SCENARIO_CONFIGS, ids=lambda x: x["id"]
)
def test_deployment_timeline_scenarios(
    ...
    commit_hashes = _get_commit_hashes_from_timeline_scenario(
    ...
    if (
        "has_successful_evaluations" in expected
        and expected["has_successful_evaluations"]
    ):
    ...
    if "has_failed_evaluations" in expected and expected["has_failed_evaluations"]:
        ...
        failed_rows = [
            r for r in rows if r["total_evaluations"] > r["successful_evaluations"]
        ]
    ...
    if "has_recent_deployments" in expected and expected["has_recent_deployments"]:
        ...
        recent_rows = [
            r
            for r in rows
            if r["first_deployment"] and r["first_deployment"] > recent_cutoff
        ]
    ...
    if "has_old_deployments" in expected and expected["has_old_deployments"]:
        ...
        old_rows = [
            r
            for r in rows
            if r["first_deployment"] and r["first_deployment"] < old_cutoff
        ]
...
def test_deployment_timeline_view_performance(cf_client: CFTestClient):
    ...
    assert (
        query_time < 15.0
    ), f"Deployment timeline view query took too long: {query_time:.2f} seconds"
    assert len(result) == 1
...
def test_deployment_timeline_30_day_filter(cf_client: CFTestClient, clean_test_data):
    ...
    assert (
        len(old_rows) == 0
    ), f"Expected no old commits (>30 days), but found {len(old_rows)}"

    # Recent commit should appear
    assert (
        len(recent_rows) >= 1
    ), f"Expected recent commit to appear, but found {len(recent_rows)}"
...
def test_deployment_approximation_logic(cf_client: CFTestClient, clean_test_data):
    ...
    deployment_time = commit_time + timedelta(
        minutes=30
    )  # Deployed 30 min after commit
