...
@pytest.mark.views
@pytest.mark.database
def test_derivation_status_breakdown_ordering(cf_client: CFTestClient):
    ...
    rows = cf_client.execute_sql(
        ...
        SELECT status_name
        FROM {VIEW_DERIVATION_STATUS_BREAKDOWN}
        ORDER BY status_name  -- This should match the view's ORDER BY display_order
        """
    )

    # Get the same data directly from derivation_statuses to compare ordering
    status_order_rows = cf_client.execute_sql(
        """
        SELECT name 
    ...
    expected_order = [row["name"] for row in status_order_rows]
    actual_order = [row["status_name"] for row in rows]

    # The view should include all statuses (even with 0 counts) in display order
    for expected_status in expected_order:
        assert (
            expected_status in actual_order
        ), f"Missing expected status '{expected_status}' in view results"
