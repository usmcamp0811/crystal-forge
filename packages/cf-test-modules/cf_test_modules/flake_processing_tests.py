...
class FlakeProcessingTests:
    ...
    def _wait_for_flake_processing(ctx: CrystalForgeTestContext) -> None:
        ...
        ctx.logger.log_success("Flake 'crystal-forge' has been processed and stored in database")
    ...
    def _verify_cf_test_sys_processing(ctx: CrystalForgeTestContext) -> None:
        ...
        if int(cf_test_sys_count) > 0:
            ctx.logger.log_success("cf-test-sys derivation successfully stored in database")
            ...
            if int(dry_run_complete_count) > 0:
                ctx.logger.log_success("cf-test-sys has been successfully dry-run evaluated")
            else:
                ctx.logger.log_warning("cf-test-sys dry-run evaluation may still be in progress")
                
        else:
            ctx.logger.log_warning("cf-test-sys derivation not found - may need longer evaluation time")
