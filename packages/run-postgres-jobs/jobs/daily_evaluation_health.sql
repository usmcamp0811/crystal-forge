INSERT INTO daily_evaluation_health (snapshot_date, total_evaluations, successful_evaluations, failed_evaluations, pending_evaluations, avg_evaluation_duration_ms, max_evaluation_duration_ms, success_rate_percentage, evaluations_with_retries)
SELECT
    ...
    ROUND(COUNT(*) FILTER (WHERE status = 'complete') * 100.0 / NULLIF (COUNT(*) FILTER (WHERE status IN ('complete', 'failed')), 0), 2) AS success_rate_percentage,
