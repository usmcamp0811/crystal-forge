...
const COMPLIANCE_BUNDLES = (typeof __fx === "function" && __fx("compliance")) || [
  ...
  },
  {
    ...
    policyIds: ["stig-sshd", "stig-auditd", "stig-banner", "stig-fips", "stig-usbguard", "stig-pwquality", "stig-sshd-root-login-disabled", "stig-sshd-password-auth-disabled", "stig-sshd-empty-passwords-denied", "stig-sshd-x11-forwarding-disabled", "stig-sshd-idle-timeout", "stig-sshd-login-grace-time", "stig-sshd-max-auth-tries", "stig-sshd-fips-macs", "stig-sshd-fips-kex-algorithms", "stig-sshd-strict-modes", "stig-sshd-host-key-algorithms", "stig-auditd-enabled", "stig-auditd-audit-backlog-limit", "stig-auditd-failure-action", "stig-auditd-log-retention", "stig-auditd-execve-rules", "stig-auditd-privileged-command-rules", "stig-auditd-account-mod-rules", "stig-auditd-sudo-log-rules", "stig-auditd-time-change-rules", "stig-auditd-perm-mod-rules", "stig-auditd-module-load-rules", "stig-auditd-immutable-ruleset", "stig-auditd-remote-offload", "stig-pam-faillock-deny", "stig-pam-faillock-unlock-time", "stig-pam-faillock-fail-interval", "stig-pwquality-minlen", "stig-pwquality-complexity-classes", "stig-pwquality-max-repeat", "stig-pwquality-dictcheck", "stig-password-hash-sha512", "stig-password-max-age", "stig-password-min-age", "stig-password-reuse-limit", "stig-accounts-no-shared-ids", "stig-accounts-inactive-disable", "stig-sudo-no-nopasswd", "stig-sudo-timestamp-timeout", "stig-sudo-use-pty", "stig-login-banner-dod-notice", "stig-ssh-banner-configured", "stig-session-idle-lock", "stig-ctrl-alt-del-disabled", "stig-kernel-aslr-enabled", "stig-kernel-ptrace-scope", "stig-kernel-dmesg-restrict", "stig-kernel-core-dumps-disabled", "stig-kernel-unprivileged-bpf-disabled", "stig-kernel-module-loading-locked", "stig-kernel-lockdown-integrity", "stig-net-ip-forward-disabled", "stig-net-source-route-disabled", "stig-net-icmp-redirects-disabled", "stig-net-reverse-path-filter", "stig-net-ipv6-ra-disabled", "stig-fs-tmp-noexec", "stig-fs-vartmp-nosuid", "stig-fs-home-nosuid", "stig-fs-removable-nosuid", "stig-fs-home-dir-permissions", "stig-fs-world-writable-dirs-sticky", "stig-fs-no-unowned-files", "stig-fs-library-permissions", "stig-fs-audit-log-permissions", "stig-disk-encryption-luks2", "stig-swap-encrypted", "stig-fips-mode-enabled", "stig-fips-openssl-provider", "stig-tls-min-version-1-2", "stig-tls-approved-cipher-suites", "stig-cert-validation-enforced", "stig-cert-revocation-checking", "stig-key-rotation-90-days", "stig-secrets-not-in-nix-store", "stig-usbguard-enabled", "stig-usbguard-default-block", "stig-usb-storage-module-blacklisted", "stig-bluetooth-disabled", "stig-wireless-interfaces-disabled", "stig-firewire-module-blacklisted", "stig-firewall-default-deny", "stig-firewall-allowed-ports-explicit", "stig-telnet-service-absent", "stig-ftp-service-absent", "stig-nfs-exports-restricted", "stig-snmp-v3-only", "stig-mail-relay-local-only", "stig-vnc-service-absent", "stig-cron-restricted-to-root", "stig-container-runtime-rootless", "stig-container-images-signed", "stig-journald-forward-to-syslog", "stig-journald-storage-persistent", "stig-logrotate-configured", "stig-time-sync-authorized-source", "stig-package-signature-verification", "stig-update-cadence-30-days", "stig-boot-loader-password", "stig-secure-boot-enforced", "stig-tpm-measured-boot", "stig-selinux-or-apparmor-enforcing", "stig-systemd-service-sandboxing", "cve-gated"],
  ...
  },
  {
    ...
    policyIds: ["cve-gated","stig-sshd-root-login-disabled","stig-sshd-password-auth-disabled","stig-sshd-idle-timeout","stig-auditd-enabled","stig-auditd-execve-rules","stig-auditd-account-mod-rules","stig-auditd-remote-offload","stig-pam-faillock-deny","stig-pam-faillock-unlock-time","stig-pwquality-minlen","stig-password-hash-sha512","stig-password-reuse-limit","stig-accounts-inactive-disable","stig-sudo-no-nopasswd","stig-login-banner-dod-notice","stig-session-idle-lock","stig-kernel-aslr-enabled","stig-fips-mode-enabled","stig-tls-min-version-1-2","stig-cert-validation-enforced","stig-firewall-default-deny","stig-package-signature-verification","stig-update-cadence-30-days","stig-time-sync-authorized-source","stig-selinux-or-apparmor-enforcing","stig-secure-boot-enforced"],
  ...
  },
  {
    ...
    digest: "sha256:c18a44",
    description: "Quality-management clauses applied to configuration delivery — change authorization, retained records, audit cadence, corrective action.",
    layer: "process",
    owner: "quality-team",
    lastReview: "2026-06-09",
    policyIds: ["iso-documented-information","iso-change-authorization","iso-change-records","iso-internal-audit-cadence","iso-corrective-action","iso-external-provider-control","iso-measurement-traceability","iso-competence-records","two-approver","business-hours","cve-gated","stig-package-signature-verification","stig-update-cadence-30-days","stig-auditd-log-retention","stig-journald-storage-persistent","stig-logrotate-configured"],
    requiredEnvs: ["production","staging"],
  },
];
...
// Cheap aggregate score for a bundle across whichever systems it currently applies to —
// used by the bundle picker so 30+ bundles can show a score dot without opening each one.
function bundleQuickStats(bundle) {
  if (typeof SYSTEMS === "undefined") return { score: null, systemCount: 0 };
  const rollups = SYSTEMS.map(s => bundleStatusForSystem(bundle, s)).filter(r => r.applies);
  if (!rollups.length) return { score: null, systemCount: 0 };
  const totals = rollups.reduce((acc, r) => { acc.pass += r.pass; acc.waiver += r.waiver; acc.total += r.total; return acc; }, { pass: 0, waiver: 0, total: 0 });
  return { score: totals.total ? Math.round((totals.pass + totals.waiver) / totals.total * 100) : null, systemCount: rollups.length };
}

Object.assign(window, { COMPLIANCE_BUNDLES, EVIDENCE_TYPES, bundleStatusForSystem, evidenceForControl, groupBundlesByLineage, resolveComplianceAssignment, COMPLIANCE_ASSIGNMENT_STATUS, bundleQuickStats });
