...
**Signature Process**:
1. Builder signs the raw request body with its private key
2. Base64-encode the signature
3. Send signature in `X-Signature` header
4. Server verifies signature using builder's stored public key
...
1. **Verify signature generation**: Ensure signing raw request body
...
4. **Check headers**: `X-Builder-ID` and `X-Signature` must be present
