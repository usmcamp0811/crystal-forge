...
pkgs.runCommand "xccdf-schema-validation" {
  nativeBuildInputs = [ pkgs.libxml2 pkgs.openscap ];
  meta = {
  ...
  # --- 5. Full combined writer output (well-formedness check only) ---
  ...
  <Benchmark xmlns="http://checklists.nist.gov/xccdf/1.2"
    ...
    <description>
      <p xmlns="http://www.w3.org/1999/xhtml">
        Complete writer-generated document combining XCCDF structure with
        CF extension elements embedded in Rules. This document is valid XML
        but cannot be schema-validated against either schema alone because
        CF elements inside Rules are not recognized by the XCCDF 1.2 schema.
      </p>
    </description>
    <version>1.0.0</version>
    <metadata>
    ...
    <Group id="xccdf_crystalforge_group_full">
      ...
      <Rule id="xccdf_crystalforge_rule_fw" role="full" severity="high">
      ...
      <Rule id="xccdf_crystalforge_rule_cc" role="full" severity="high">
  ...
  # --- Validation: Full writer output (well-formedness only) ---
  # This document contains CF elements inside Rules, which the XCCDF 1.2
  # schema does not allow (no ##any extension point in ruleType). We validate
  # well-formedness to catch structural XML errors.
  xmllint --noout full-writer-output.xml
