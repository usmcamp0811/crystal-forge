{
  ...
  prefetchNode =
    name: node:
    let l = node.locked or {}; in
    if (l.type or "") == "github" then
      {
        ...
        from = { type = "github"; owner = l.owner; repo = l.repo; };
      }
    else if (l.type or "") == "git" then
      {
        key  = "git:${l.url}";
        path = builtins.fetchTree {
          type    = "git";
        ...
        from = { type = "git"; url = l.url; };
      }
    else if (l.type or "") == "tarball" then
      {
        key  = "tarball:${l.url}";
        path = builtins.fetchTree {
          type    = "tarball";
  ...
  registryEntries =
    lib.listToAttrs (map
      (x: lib.nameValuePair
        ...
        { from = x.from; to = { type = "path"; path = x.path; }; })
      prefetchedList);
in
pkgs.testers.runNixOSTest {
  ...
  nodes = {
    testNode = { pkgs, ... }: {
      ...
      virtualisation.additionalPaths = [ testFlake pkgs.path ] ++ prefetchedPaths;
  ...
  testScript = ''
    ...
    testNode.succeed(f"echo 'FLAKE SHOW OUTPUT:' >> {report_file}")
    testNode.succeed(f"echo '==================' >> {report_file}")
    testNode.succeed(f"cd {work_dir} && nix flake show --offline >> {report_file} 2>&1 || echo 'flake show failed' >> {report_file}")
    testNode.succeed(f"echo >> {report_file}")

    testNode.succeed(f"echo 'FLAKE METADATA:' >> {report_file}")
    testNode.succeed(f"echo '===============' >> {report_file}")
    testNode.succeed(f"cd {work_dir} && nix flake metadata --offline >> {report_file} 2>&1 || echo 'flake metadata failed' >> {report_file}")
    testNode.succeed(f"echo >> {report_file}")

    testNode.succeed(f"echo 'BUILD TEST:' >> {report_file}")
    testNode.succeed(f"echo '===========' >> {report_file}")
    ...
    testNode.succeed(f"[ -e /tmp/flake-build ] || (echo 'Packages available:' >> {report_file}; cd {work_dir}; nix flake show --json --offline 2>/dev/null | jq -r '..|.packages? // empty | to_entries[] | .key' >> {report_file} || true)")
    testNode.succeed(f"echo >> {report_file}")
